...
public class EmojiSelectionGui extends Overlay   {
    ...
    public void render(PoseStack stack, int i, int j, float f) {
        ...
        if (this.showingSelectionArea){
            ...
            for (int is = 0; is < 6; is++) {
                drawLine(stack, is * 12f, is + selectionPointer);
            }
            ...
            if (lastEmoji != null){
                ...
                float ig = -iTextPropertiesList.size() / 2;
                stack.pushPose();
                ...
                for (FormattedCharSequence reorderingProcessor : iTextPropertiesList) {
                    ...
                    EmojiClientProxy.oldFontRenderer.draw(stack, stringBuilder.toString(), (emojiInfoArea.getX() + 15) * (1/textScale), (emojiInfoArea.getY() + 8 + 4 * ig)  * (1/textScale), 0x969696);
                    ++ig;
                }
                stack.scale(1,1,1);
                stack.popPose();
            ...
            for (int iv = 0; iv < 7; iv++) {
                int selCategory = iv + categoryPointer;
                if (selCategory < EmojiClientProxy.CATEGORIES.size()){
                    EmojiCategory category = EmojiClientProxy.CATEGORIES.get(selCategory);
                    Rect2i rec = new Rect2i(categorySelectionArea.getX() + 6, categorySelectionArea.getY() + 6 + iv * 12, 11, 11);
                    ...
                    if (EmojiClientProxy.SORTED_EMOJIS_FOR_SELECTION.containsKey(category) && EmojiClientProxy.SORTED_EMOJIS_FOR_SELECTION.get(category).size() > 0){
                        Minecraft.getInstance().font.draw(stack, EmojiClientProxy.SORTED_EMOJIS_FOR_SELECTION.get(category).get(0)[0].strings.get(0), categorySelectionArea.getX() + 6, categorySelectionArea.getY() + 6 + iv * 12, 0);
                    }
                }
            }
