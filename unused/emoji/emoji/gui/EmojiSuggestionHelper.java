...
public class EmojiSuggestionHelper     {
        ...
        if (stringreader.canRead()) {
            ...
            if (lastWordIndex < s.length() ? s.charAt(lastWordIndex) == ':' : s.length() > 0 && s.charAt(0) == ':')
                if ((skip || cursorPosition - lastWordIndex >= 3) && (this.suggestions == null || !this.updating)) {
                    final CompletableFuture<Iterable<String>> list = CompletableFuture.supplyAsync(() -> EmojiClientProxy.ALL_EMOJIS);
        ...
        if (this.suggestionsFuture != null && this.suggestionsFuture.isDone()) {
            ...
            if (!suggestions.getList().isEmpty()) {
                for (final Suggestion suggestion : suggestions.getList())
                    i = Math.max(i, EmojiClientProxy.oldFontRenderer.width(suggestion.getText()));
    ...
    public class EmojiSuggestions {
        ...
        public EmojiSuggestions(int x, int y, int areaWidth, Suggestions suggestions) {
            ...
            int height = Math.min(suggestions.getList().size(), 10) * (EmojiClientProxy.oldFontRenderer.lineHeight + 3);
        ...
        public void render(PoseStack stack){
            for (int i = 0; i < Math.min(this.suggestions.getList().size(), 10); ++i) {
                ...
                EmojiClientProxy.oldFontRenderer.drawShadow(stack, suggestion.getText(), 12 + this.area.getX() + 1, this.area.getY() + 2 + 12 * i, pos == this.index ? 0xFFFFFF00 : 0xFFAAAAAA);
            }
        }
